...
- Mana aspects: Dawn, Tide, Grave, Ember, Grove. Cards cost generic mana plus colored pips. Colored pips are paid first; automatic tapping prefers basics before flexible duals. Players can tap explicitly to plan payment. Tapping a source creates floating mana; it empties at phase/turn boundaries.
- Non-instants and Hero abilities require the active player's main phase and empty stack. Instants may be cast during response windows and combat. Cast spells and activated Hero abilities enter a visible LIFO stack; targets are rechecked on resolution. `Resolve top of stack` resolves one item, retaining a response window before the next. Enemy actions advance through visible priority passes.
...
- Life at zero loses; attempting to draw from an empty library loses. If both hearths reach zero together, the current prototype records a player loss. Victories award 35 crowns and 5 renown once. Conceding is free.
