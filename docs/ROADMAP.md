...
- [ ] **In progress — Add The Quiet, a thirteenth 80-card volume** (the-quiet)
  ...
  - Acceptance still needed: Remaining: merge the feature branch and publish the checksummed provenance archive for the new sources, then rerun production-path checks on the merged revision. Human balance remains a separate release gate.
