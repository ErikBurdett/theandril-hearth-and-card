...
- 120 unit/content/art tests across 21 files, including the new set, judgment-freshness and catalog-summary tests;
- all 48 Chromium gameplay scenarios (45 existing plus 3 new);
...
The preceding helper increment used gameplay baseline `841cd94`: 8 sets / 640 cards, 18 prepared recipes, 20 named visitors using 14 visual families, and save schema 3. Battle polish, local-save recovery and guest challenges described below are implemented. Deeper combat rules, individual strategic AI profiles, tactical tutorials, long-term balance, real-device coverage and full 1.0 acceptance remain open. Detailed sections below retain earlier increments and their scoped historical measurements; they are not an accumulating full-suite total or a substitute for the current roadmap.
