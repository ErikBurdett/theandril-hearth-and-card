...
No Aseprite, Pixel Snapper, model name or seed is claimed. Runtime files are lossless WebP derivatives, pixel-compared to the approved PNGs. Originals, generation records, rejected sources, candidates and review sheets are gitignored for a future checksum-pinned archive. That archive has not been uploaded, so `art:restore` cannot yet rebuild this set's sources on another machine.
