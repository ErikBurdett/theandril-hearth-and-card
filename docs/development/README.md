...
3. Link the exact reviewed source revision and evidence. Earlier gameplay claims are pinned to `841cd94`; Living Cultures catalogue, art, collection, recipe and lore entries are pinned to `b039bd3`. The helper's own source links use main. Move an item to Completed only when its named behavior is verified, and never infer overall release acceptance from a demo deployment.
