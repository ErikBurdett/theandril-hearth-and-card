...
if (command === "prepare") {
  for (const family of ["visitors"]) {
    ...
    // Deterministic chroma-key extraction of the retained magenta source.
    for (let at = 0; at < sheet.data.length; at += 4) {
      const [r, g, b] = sheet.data.subarray(at, at + 3);
    ...
    for (let i = 0; i < familyIds.length; i++) {
      const id = familyIds[i],
        ...
        rows = 2,
        cw = sheet.width / cols,
        ch = i < cols ? 490 : sheet.height - 490,
        x0 = (i % cols) * cw,
        y0 = i < cols ? 0 : 490;
      let left = cw,
        top = ch,
      ...
      const manifest: AssetManifest = {
        ...
        provenance: {
          ...
          licenseNotes: [
            "Original generated tavern visitor; a Hearth & Card addition, not a canonical historical portrait.",
          ],
        },
        ...
        processing: [
          {
            ...
            version: "1",
            profile: "registered-96x128-magenta-key-alpha220-nearest",
            settingsHash: sha256(
              JSON.stringify({
                key: "r,b>130;g<min(r,b)*0.55",
                left,
                top,
                right,
                bottom,
