...
For new or edited authored card text (sets with `authored: true`), run `npm run cards:judge -- judge SET_ID` before building briefs or commissioning art, and resolve blocking flags. The TypeSafe gate needs `TYPESAFE_API_KEY` or `~/.config/typesafe/api-key`; never commit the key. `tests/card-judgments.test.ts` fails when judgments are stale or when any judged card carries a blocking flag. Questions are scoped by era and by whether a set is authored; see `questionScope` in `scripts/card-judgment-state.ts` before adding one. TypeSafe reads text only; visual review stays the image gate. See `docs/THE_QUIET.md`.
